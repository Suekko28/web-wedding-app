...
@section('pageContent')
    ...
    @include('layouts.breadcrumb', ['title' => 'Wedding Design 6', 'subtitle' => 'Dashboard'])
                    ...
                    <tbody>
                        ...
                        @foreach ($data as $item)
                            <tr class="text-center">
                                ...
                                <td>
                                    @if ($item->KontenDesign6->isEmpty())
                                        <a class="btn btn-primary"
                                            href="{{ route('form-design6.create', ['id' => $item->id]) }}">Buat
                                            Konten</a>
                                    @else
                                        <a class="btn btn-secondary"
                                            href="{{ route('form-design6.edit', ['informasiDesign6Id' => $item->KontenDesign6->first()->informasi_design6_id, 'id' => $item->KontenDesign6->first()->id]) }}">
                                            Edit Konten
                                        </a>
                                    @endif
                                </td>
                                <td>
                                    @if ($item->KontenDesign6->isNotEmpty())
                                        <a class="btn btn-primary"
                                            href="{{ route('nama-undangan-list6', ['weddingDesign6Id' => $item->KontenDesign6->first()->id]) }}"
                                            target="_blank">
                                            Buat Tamu
                                        </a>
                                    @else
                                ...
                                <td>
                                    <div class="btn-group-horizontal">
                                        ...
                                        <a href="{{ $item->KontenDesign6->first() && $item->KontenDesign6->first()->id
                                            ? route('wedding-design6.show', ['wedding_design6' => $item->KontenDesign6->first()->id])
                                            : '#' }}"
                                            class="btn btn-info rounded mb-2 
                                          {{ !$item->KontenDesign6->first() || !$item->KontenDesign6->first()->nama_mempelai_laki || !$item->KontenDesign6->first()->nama_mempelai_perempuan ? 'disabled' : '' }}">
                                            <i class="fa fa-circle-info" style="color:white;"></i>
                                        </a>
                                        @php
                                            $isDisabled =
                                                !$item->KontenDesign6->first() ||
                                                !$item->slug_nama_pasangan ||
                                                !$item->id_weddingdesign6;

                                            // Ambil hanya angka belakang dari ID, misal: 0001
                                            $matches = [];
                                            preg_match('/WDDS6-\d{8}-(\d+)/', $item->id_weddingdesign6, $matches);
                                            $id_angka = $matches[1] ?? '0000'; // hanya 0001, tanpa WDDS6 prefix
                                        ...
                                        <a class="btn btn-primary mb-2 {{ $isDisabled ? 'disabled' : '' }}"
                                            href="{{ $isDisabled
                                                ? '#'
                                                : route('wedding-design6-home-preview', [
                                                    'id_weddingdesign6' => $id_angka,
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formBuatUndangan" action="{{ route('wedding-design6.store') }}" method="POST">
                        @csrf
    ...
    <script>
        ...
        document.getElementById('btnBuatUndangan').addEventListener('click', function() {
            ...
            document.getElementById('formBuatUndangan').action = "{{ route('wedding-design6.store') }}";
        ...
        document.querySelectorAll('.edit-btn').forEach(function(button) {
            button.addEventListener('click', function(event) {
                ...
                document.getElementById('formBuatUndangan').action = '/wedding-design6/' + id + '/update';
