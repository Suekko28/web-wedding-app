...
@section('pageContent')
    ...
    @include('layouts.breadcrumb', ['title' => 'Wedding Design 7', 'subtitle' => 'Dashboard'])
                    ...
                    <tbody>
                        ...
                        @foreach ($data as $item)
                            <tr class="text-center">
                                ...
                                <td>
                                    @if ($item->KontenDesign7->isEmpty())
                                        <a class="btn btn-primary"
                                            href="{{ route('form-design7.create', ['id' => $item->id]) }}">Buat
                                            Konten</a>
                                    @else
                                        <a class="btn btn-secondary"
                                            href="{{ route('form-design7.edit', ['informasiDesign7Id' => $item->KontenDesign7->first()->informasi_design7_id, 'id' => $item->KontenDesign7->first()->id]) }}">
                                            Edit Konten
                                        </a>
                                    @endif
                                </td>
                                <td>
                                    @if ($item->KontenDesign7->isNotEmpty())
                                        <a class="btn btn-primary"
                                            href="{{ route('nama-undangan-list7', ['weddingDesign7Id' => $item->KontenDesign7->first()->id]) }}"
                                            target="_blank">
                                            Buat Tamu
                                        </a>
                                    @else
                                ...
                                <td>
                                    <div class="btn-group-horizontal">
                                        ...
                                        <a href="{{ $item->KontenDesign7->first() && $item->KontenDesign7->first()->id
                                            ? route('wedding-design7.show', ['wedding_design7' => $item->KontenDesign7->first()->id])
                                            : '#' }}"
                                            class="btn btn-info rounded mb-2 
                                          {{ !$item->KontenDesign7->first() || !$item->KontenDesign7->first()->nama_mempelai_laki || !$item->KontenDesign7->first()->nama_mempelai_perempuan ? 'disabled' : '' }}">
                                        ...
                                        <a class="btn btn-primary mb-2 {{ !$item->KontenDesign7->first() || !$item->KontenDesign7->first()->nama_mempelai_laki || !$item->KontenDesign7->first()->nama_mempelai_perempuan ? 'disabled' : '' }}"
                                            href="{{ route('wedding-design7-home-preview', [
                                                'nama_mempelai_laki' => $item->KontenDesign7->first()->nama_mempelai_laki ?? 'Unknown',
                                                'nama_mempelai_perempuan' => $item->KontenDesign7->first()->nama_mempelai_perempuan ?? 'Unknown',
                                            ]) }}"
                                            target="_blank">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formBuatUndangan" action="{{ route('wedding-design7.store') }}" method="POST">
                        @csrf
    ...
    <script>
        ...
        document.getElementById('btnBuatUndangan').addEventListener('click', function() {
            ...
            document.getElementById('formBuatUndangan').action = "{{ route('wedding-design7.store') }}";
        ...
        document.querySelectorAll('.edit-btn').forEach(function(button) {
            button.addEventListener('click', function(event) {
                ...
                document.getElementById('formBuatUndangan').action = '/wedding-design7/' + id + '/update';
