...
@section('pageContent')
    ...
    @include('layouts.breadcrumb', ['title' => 'Wedding Design 5', 'subtitle' => 'Dashboard'])
                    ...
                    <tbody>
                        ...
                        @foreach ($data as $item)
                            <tr class="text-center">
                                ...
                                <td>
                                    @if ($item->KontenDesign5->isEmpty())
                                        <a class="btn btn-primary"
                                            href="{{ route('form-design5.create', ['id' => $item->id]) }}">Buat
                                            Konten</a>
                                    @else
                                        <a class="btn btn-secondary"
                                            href="{{ route('form-design5.edit', ['informasiDesign5Id' => $item->KontenDesign5->first()->informasi_design5_id, 'id' => $item->KontenDesign5->first()->id]) }}">
                                            Edit Konten
                                        </a>
                                    @endif
                                </td>
                                <td>
                                    @if ($item->KontenDesign5->isNotEmpty())
                                        <a class="btn btn-primary"
                                            href="{{ route('nama-undangan-list5', ['weddingDesign5Id' => $item->KontenDesign5->first()->id]) }}"
                                            target="_blank">
                                            Buat Tamu
                                        </a>
                                    @else
                                ...
                                <td>
                                    <div class="btn-group-horizontal">
                                        ...
                                        <a href="{{ $item->KontenDesign5->first() && $item->KontenDesign5->first()->id
                                            ? route('wedding-design5.show', ['wedding_design5' => $item->KontenDesign5->first()->id])
                                            : '#' }}"
                                            class="btn btn-info rounded mb-2 
                                          {{ !$item->KontenDesign5->first() || !$item->KontenDesign5->first()->nama_mempelai_laki || !$item->KontenDesign5->first()->nama_mempelai_perempuan ? 'disabled' : '' }}">
                                            <i class="fa fa-circle-info" style="color:white;"></i>
                                        </a>
                                        @php
                                            $isDisabled =
                                                !$item->KontenDesign5->first() ||
                                                !$item->slug_nama_pasangan ||
                                                !$item->id_weddingdesign5;

                                            // Ambil hanya angka belakang dari ID, misal: 0001
                                            $matches = [];
                                            preg_match('/WDDS5-\d{8}-(\d+)/', $item->id_weddingdesign5, $matches);
                                            $id_angka = $matches[1] ?? '0000'; // hanya 0001, tanpa WDDS5 prefix
                                        ...
                                        <a class="btn btn-primary mb-2 {{ $isDisabled ? 'disabled' : '' }}"
                                            href="{{ $isDisabled
                                                ? '#'
                                                : route('wedding-design5-home-preview', [
                                                    'id_weddingdesign5' => $id_angka,
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formBuatUndangan" action="{{ route('wedding-design5.store') }}" method="POST">
                        @csrf
    ...
    <script>
        ...
        document.getElementById('btnBuatUndangan').addEventListener('click', function() {
            ...
            document.getElementById('formBuatUndangan').action = "{{ route('wedding-design5.store') }}";
        ...
        document.querySelectorAll('.edit-btn').forEach(function(button) {
            button.addEventListener('click', function(event) {
                ...
                document.getElementById('formBuatUndangan').action = '/wedding-design5/' + id + '/update';
